...
#if defined(NDEBUG)
# error "Tnmcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc47a482110c8b07bcfe8936ba2daeef5c8d7d41b131fed088d3657f3502fcb29");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Tnmcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Tnmcoin Signed Message:\n";
...
{
    // Tnmcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Tnmcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 4 * 24 * 60 * 60; // Tnmcoin: 4 days
static const int64 nTargetSpacing = 2 * 60; // Tnmcoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Tnmcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Tnmcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x64f929241c5fdebd3d8d5d235e1587037cd66a331731c004809c56e6c96bbb17");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "10/aout/2020 Tnmcoin A Blockchain ecosystem ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 60 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a96ec95097a341701810590a1b205bd677b71fc3729731f500a3c48bff76405a10e823b6e680cf90d9fb435634cb7194d87b06410427c9f239c5d4ef6fb7cc6a") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1597061843;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085022666;

        if (fTestNet)
        {
            block.nTime    = 1597061775;
            block.nNonce   = 386346273;
        }
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x4acff7fd835ee92f61802140a8432eaf5a7de681d041677ef00d51e015107b33"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb7, 0xdf }; // Tnmcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TnmcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TnmcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TnmcoinMiner(CWallet *pwallet)
{
    printf("TnmcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tnmcoin-miner");
    ...
    try { loop {
        ...
        printf("Running TnmcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TnmcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TnmcoinMiner, pwallet));
}
